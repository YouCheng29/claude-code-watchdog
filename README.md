...
支援 **macOS 與 Linux**（date 已做相容層；caffeinate / 桌面通知為 macOS 專屬、Linux 自動略過）。
...
# 2) 放到固定資料夾
mkdir -p ~/.local/bin/ccw
cp ccw.sh _ccw-watch.sh _resume-lib.sh ~/.local/bin/ccw/
chmod +x ~/.local/bin/ccw/ccw.sh ~/.local/bin/ccw/_ccw-watch.sh

# 3) 加 alias（zsh 用 ~/.zshrc、bash 用 ~/.bashrc）
echo 'alias ccw="$HOME/.local/bin/ccw/ccw.sh"' >> ~/.zshrc
# 開新終端生效
```
...
`sleep` 到重置 +5 分 → `send-keys「繼續」Enter` → 桌面通知。
