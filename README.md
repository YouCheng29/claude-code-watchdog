...
> If the project has pending todos (`TODO.md`, etc.), it instead types an instruction to read the todo and continue from the first incomplete item — useful when you hit the limit before work even started.
...
續跑有兩種模式（看門狗自己**不讀檔案內容**，只檢查待辦檔是否存在、是否有未完成項，然後把對應指令**打字送進 session**，由 Claude 自己去讀）：

| 情況 | 看門狗送什麼 |
|---|---|
| 專案裡**沒有**未完成待辦 | 「繼續」——接回原本對話 |
| 專案裡**有**未完成待辦（`TODO.md` 等） | 「先讀 `TODO.md`、對照 session 上下文、從第一個未完成項接續做」 |

**待辦從哪來？** 看門狗從 tmux session 的**當前工作目錄**（你 `cd` 進專案後跑 `ccw` 的那個目錄）依序找：

1. `CCW_TODO_FILE`（環境變數，若有設）
2. 專案根：`TODO.md` → `todo.md` → `.ccw-todo.md`
3. `~/.claude/projects/<專案slug>/memory/*-status.md`（字母順，第一個有未完成項者勝出）

檔案裡要有未勾選的 `- [ ]`（或 `- [⏸]`）才會觸發。

**典型場景**：白天一開工就撞牆、程式還沒跑過——沒關係，先把任務寫進 `TODO.md`，`ccw` 後 detach。晚上額度重置，看門狗醒來後不會只丟一句「繼續」，而是叫 Claude **自己去讀 todo 開工**。

```sh
cd 你的專案目錄          # 例如青年資委/僑外生留台講座
# 確認根目錄有 TODO.md，且含 - [ ] 未完成項
ccw                    # detach（Ctrl-B D）後即可離開
```

**待辦在子目錄、或要換掉舊的 memory checkpoint？** 計劃若寫在 `docs/exec-plans/…` 等深層路徑，或 `memory/*-status.md` 裡還有舊的 `- [⏸]` 會搶先被讀到，用下面任一方式指定：

| 作法 | 何時用 | 範例 |
|---|---|---|
| **`.ccw-todo.md` symlink**（推薦） | 已跑的 session 也能生效，不用重開 ccw | `ln -sf docs/exec-plans/my-plan-todolist.md .ccw-todo.md` |
| **`CCW_TODO_FILE`** | 新開 session 時指定 | `CCW_TODO_FILE=docs/exec-plans/my-plan-todolist.md ccw` |

symlink 會優先於 memory 的 `*-status.md`；換計劃時記得把舊 checkpoint 的 `- [⏸]` 勾成完成或結案，避免日後拿掉 symlink 又誤觸。
...
| `_ccw-todo.sh` | 撞牆恢復前：掃專案待辦（`TODO.md` 等），有待辦則送 todo-aware 續跑訊息 |
...
ccw status        # 總覽：活著的 session（含專案路徑）、看門狗、log 尾巴
ccw clean         # 清掉殘留 session 與孤兒看門狗
```

離開時撞牆凍住 → 看門狗確認畫面真的靜止後，等額度重置、自動送「繼續」，並發桌面通知。
若專案裡有**未完成的待辦**（`TODO.md`、`.ccw-todo.md`，或 Claude memory 的 `*-status.md`），
會改送「先讀待辦、對照 session 上下文、從未完成項接續」的訊息——適合白天撞牆、晚上額度重置後無人值守接續。
log：`~/.claude/ccw.log`。
...
`sleep` 到重置 +5 分 → 先送 `Escape` 清可能殘留的對話框 →
掃專案待辦（`TODO.md` 等）→ 有待辦則送「先讀 todo + 對照 session 再接續」，否則 `send-keys「繼續」Enter` → 桌面通知。
...
| `CCW_CONTINUE_MSG` | 繼續 | 撞牆重置後打進去的字（**無待辦時**） |
| `CCW_TODO_FILE` | （自動找） | 指定待辦檔路徑（可相對專案根目錄） |
| `CCW_TODO_RESUME_MSG` | （見腳本） | 有待辦時的續跑訊息；`{todo}` = 檔名或路徑 |
...
**它只會做兩件事**：讀 tmux 畫面（`capture-pane`）、送固定按鍵（`Escape` + 續跑訊息 + `Enter`）。續跑訊息為「繼續」或 todo-aware 句，視專案待辦而定。**它不呼叫 Claude API、不讀取待辦檔內容、不執行任何 shell 指令、不碰網路、不碰你的 secret。**
...
- 看門狗送出的 `CCW_CONTINUE_MSG` / `CCW_TODO_RESUME_MSG`（預設「繼續」或 todo-aware 續跑句）會被送進你的 agent。**別把它設成有殺傷力的指令**——它會在你不在時被自動送出。
...
- 升級選單 fail-safe（v0.1.3）：選單 Escape 後仍在 → 不送 Enter、改通知 ✅ 實測；正常路徑不受影響 ✅ 實測
...
- 撞牆時 Claude 另跳升級選單（`What do you want to do? 1. Upgrade your plan / 2. Upgrade to Team / 3. Stop and wait`，預設反白 Upgrade）。**v0.1.3 起有 fail-safe**：送 Enter 前先 `Escape`，再確認選單是否消失——**選單還在就絕不送 Enter**（避免落在預設 Upgrade），改發通知請你手動選「Stop and wait」。最壞情況（誤選付費）已消除。（「Escape 成功取消選單後自動續」這條 happy path 仍待下次真撞牆蓋章；本次首撞是使用者手動選 3 後才續的。）
