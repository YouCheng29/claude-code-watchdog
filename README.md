...
- 撞牆時 Claude 另跳升級選單（`What do you want to do? 1. Upgrade your plan / 2. Upgrade to Team / 3. Stop and wait`，預設反白 Upgrade）。本次是**使用者手動選了「3. Stop and wait」**後看門狗才送繼續、成功恢復。⚠️ **「選單留著、由看門狗直接面對」這條路徑尚未實測**：`Escape` 前置理論上會取消它，但失敗時 `Enter` 可能落在預設的 Upgrade。列為待強化（見 Issues）。
